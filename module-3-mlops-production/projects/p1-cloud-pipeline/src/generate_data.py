"""Generate synthetic churn dataset for the cloud pipeline project.

Creates an 8,000-row dataset with realistic patterns and intentional
quality issues (nulls, duplicates) to practice data validation and
cleaning in the pipeline.

Key relationships (same as Module 2):
- High monthly charges + short tenure → more churn
- Month-to-month contract → more churn
- No online security/tech support → more churn
"""
...
def generate_churn_data(n_customers: int = 8000, seed: int = 42) -> pd.DataFrame:
    ...
    total_charges = np.round(
        monthly_charges * tenure * (1 + np.random.normal(0, 0.05, n_customers)), 2
    )
    ...
    # Inject messiness for pipeline practice
    # Missing values (~2% in charges columns)
    mask = np.random.random(n_customers) < 0.02
    df.loc[mask, 'total_charges'] = np.nan
    mask2 = np.random.random(n_customers) < 0.015
    df.loc[mask2, 'monthly_charges'] = np.nan

    # Duplicate rows (~1%)
    n_dupes = int(n_customers * 0.01)
